...
def get_env_bool(name: str, default: bool = True) -> bool:
    ...
    if raw_value in {"1", "true", "yes", "y", "on", "si", "sí"}:
        return True
...
LOG_CHANNEL_ID = get_env_int("LOG_CHANNEL_ID")  # Canal operativo para registros/auditoría visible
ADMIN_IDS_STR = os.getenv("ADMIN_IDS", "").strip()
ADMIN_IDS = parse_admin_ids(ADMIN_IDS_STR)
PERSISTENT_STORAGE_PATH = os.getenv("PERSISTENT_STORAGE_PATH", ".").strip()
BOT_START_TIME = datetime.now(timezone.utc)
LOG_MEMBER_EVENTS = get_env_bool("LOG_MEMBER_EVENTS", True)
LOG_MODERATION_EVENTS = get_env_bool("LOG_MODERATION_EVENTS", True)
LOG_MEMBERSHIP_EVENTS = get_env_bool("LOG_MEMBERSHIP_EVENTS", True)
LOG_ADMIN_CHANGE_EVENTS = get_env_bool("LOG_ADMIN_CHANGE_EVENTS", True)
LOG_SECURITY_EVENTS = get_env_bool("LOG_SECURITY_EVENTS", True)
LOG_BACKUP_EVENTS = get_env_bool("LOG_BACKUP_EVENTS", True)
LOG_ERROR_EVENTS = get_env_bool("LOG_ERROR_EVENTS", True)
MAX_LOG_MESSAGE_LENGTH = get_env_int("MAX_LOG_MESSAGE_LENGTH", 3500)
...
IDINFO_PAGE_SIZE = 5
IDINFO_MAX_KNOWN_CHATS = get_env_int("IDINFO_MAX_KNOWN_CHATS", 100)
SUPER_ADMIN_ID = get_env_int("SUPER_ADMIN_ID", 6060544328)
MEMBERS_PAGE_SIZE = get_env_int("MEMBERS_PAGE_SIZE", 5)
NEW_MEMBER_APPROVAL_TIMEOUT_SECONDS = get_env_int("NEW_MEMBER_APPROVAL_TIMEOUT_SECONDS", 60)
AUTO_LINK_MUTE_MINUTES = get_env_int("AUTO_LINK_MUTE_MINUTES", 60)
DAILY_REPORT_HOUR = get_env_int("DAILY_REPORT_HOUR", 23)
DAILY_REPORT_MINUTE = get_env_int("DAILY_REPORT_MINUTE", 55)
MUTE_DURATION_RE = re.compile(r"^(\d+)([mhd])$", re.IGNORECASE)
BOT_ADMIN_STATUSES = {ChatMember.ADMINISTRATOR, ChatMember.OWNER}
...
HEAVY_COMMANDS = {"archivo", "importar", "limpieza", "scan", "todosmas", "todosmenos", "mensaje", "estado", "ver", "setplan", "listar", "idinfo", "miembros", "plantillas", "deshacerimportacion", "config"}
ALLOWED_UPDATE_TYPES = [Update.MESSAGE, Update.CALLBACK_QUERY, Update.CHAT_MEMBER, Update.MY_CHAT_MEMBER]
...
EXPORT_DATASETS = {
    ...
    "log_events": {
        ...
        "table": "log_events",
        "import_key": "log_events",
        "filename_prefix": "canal_registro",
        "columns": ("id", "event_type", "event_key", "actor_user_id", "target_user_id", "chat_id", "message", "created_at", "delivered", "delivered_message_id", "delivery_error"),
        "where": "",
        "order_by": "ORDER BY id DESC",
    },
    "bot_chat_registry": {
        "label": "Chats registrados del bot",
        "table": "bot_chat_registry",
        "import_key": "bot_chat_registry",
        "filename_prefix": "chats_registrados_bot",
        "columns": ("chat_id", "title", "chat_type", "username", "bot_status", "is_admin", "can_post_messages", "can_invite_users", "can_restrict_members", "can_delete_messages", "can_promote_members", "first_seen_at", "last_seen_at", "last_admin_check_at", "active", "source", "last_error"),
        "where": "",
        "order_by": "ORDER BY is_admin DESC, title COLLATE NOCASE, chat_id",
    },
}
...
VALID_CALLBACK_PATTERNS = (
    ...
    re.compile(r"^idinfo_page_\d{1,4}$"),
    re.compile(r"^idinfo_chat_-?\d{1,20}$"),
    re.compile(r"^idinfo_close$"),
    re.compile(r"^miembros_cat_[a-z_]{1,32}_\d{1,4}$"),
    re.compile(r"^miembros_detail_\d{1,20}$"),
    re.compile(r"^miembros_add_\d{1,20}_(1|7|15|30)$"),
    re.compile(r"^miembros_remove_\d{1,20}$"),
    re.compile(r"^miembros_close$"),
    re.compile(r"^info_blacklist_add_\d{1,20}$"),
    re.compile(r"^info_blacklist_remove_\d{1,20}$"),
    re.compile(r"^rangos_close$"),
    re.compile(r"^plantilla_[a-z0-9_]{1,48}$"),
    re.compile(r"^plantillas_close$"),
    re.compile(r"^config_toggle_[a-z0-9_]{1,48}$"),
    re.compile(r"^config_close$"),
)
...
async def security_guard_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_valid_callback_data(query.data):
        ...
        raise ApplicationHandlerStop
    if query.data.startswith(("estado_", "admin_menu_", "ver_", "setplan_", "listar_", "archivo_", "importar_", "idinfo_", "miembros_", "info_blacklist_", "plantilla_", "plantillas_")) and user_id not in ADMIN_IDS:
        ...
        raise ApplicationHandlerStop
    if query.data.startswith("config_") and user_id != SUPER_ADMIN_ID:
        await query.answer("Esta configuración solo puede cambiarla el super administrador.", show_alert=True)
        logger.warning("Callback de configuración bloqueado: user_id=%s data=%s", user_id, query.data)
        raise ApplicationHandlerStop
...
def init_db():
    ...
    try:
        with get_db_connection() as conn:
            ...
            """)
            c.execute("""
                CREATE TABLE IF NOT EXISTS bot_chat_registry (
            ...
            """)
            c.execute("""
                CREATE INDEX IF NOT EXISTS idx_log_events_type_created
                ON log_events (event_type, created_at)
            """)
            c.execute("""
                CREATE INDEX IF NOT EXISTS idx_bot_chat_registry_admin_active
                ON bot_chat_registry (is_admin, active, chat_type)
            """)
            c.execute("""
                CREATE INDEX IF NOT EXISTS idx_bot_chat_registry_last_seen
                ON bot_chat_registry (last_seen_at)
            """)
            c.execute("""
...
def is_super_admin_user(user_id: int | None) -> bool:
    """Permiso sensible para configuración operativa del bot."""
    return bool(user_id and user_id == SUPER_ADMIN_ID)

async def check_super_admin_permissions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    user = update.callback_query.from_user if update.callback_query else update.effective_user
    if user and is_super_admin_user(user.id):
        return True
    if update.callback_query:
        await update.callback_query.answer("Solo el super administrador puede usar este panel.", show_alert=True)
    elif update.effective_message:
        await update.effective_message.reply_text("❌ Este panel solo puede usarlo el super administrador.")
    return False

DEFAULT_BOT_SETTINGS = {
    "mute_new_members_until_approval": "true",
    "blacklist_auto_ban": "true",
    "link_guard_enabled": "true",
    "daily_report_enabled": "true",
    "strict_approval_enabled": "true",
    "allowed_link_domains": "",
}

def get_bot_setting(key: str, default: str | None = None) -> str:
    fallback = DEFAULT_BOT_SETTINGS.get(key, default if default is not None else "")
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
        ...
        logger.error("Error consultando configuración %s: %s", key, safe_error_text(exc))
        return fallback

def get_bot_setting_bool(key: str, default: bool = True) -> bool:
    raw_value = get_bot_setting(key, "true" if default else "false")
    return str(raw_value).strip().lower() in {"1", "true", "yes", "on", "si", "sí"}

def set_bot_setting(key: str, value: str, admin_id: int) -> None:
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute(
            """
            INSERT OR REPLACE INTO bot_settings (key, value, updated_by_admin_id, updated_at)
            VALUES (?, ?, ?, ?)
            """,
            (key, value, admin_id, datetime.now(timezone.utc).isoformat()),
        )
        conn.commit()

def get_allowed_link_domains() -> set[str]:
    raw_domains = get_bot_setting("allowed_link_domains", "")
    domains = set()
...
# Canal de registro operativo: lógica interna preparada para futura modularización
LOG_CATEGORY_FLAGS = {
    "member": lambda: LOG_MEMBER_EVENTS,
    "moderation": lambda: LOG_MODERATION_EVENTS,
    "membership": lambda: LOG_MEMBERSHIP_EVENTS,
    "admin_change": lambda: LOG_ADMIN_CHANGE_EVENTS,
    "security": lambda: LOG_SECURITY_EVENTS,
    "backup": lambda: LOG_BACKUP_EVENTS,
    "error": lambda: LOG_ERROR_EVENTS,
}

ADMIN_PRIVILEGE_FIELDS = {
    "can_be_edited",
    "can_manage_chat",
    "can_delete_messages",
...
def is_log_category_enabled(category: str | None) -> bool:
    if not category:
        return True
    checker = LOG_CATEGORY_FLAGS.get(category)
    return True if checker is None else bool(checker())


def format_log_datetime(value: datetime | None = None) -> str:
    dt = value or datetime.now(timezone.utc)
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)
    return dt.astimezone(BOT_TIMEZONE).strftime("%d.%m.%y a las %I:%M:%S %p")


def format_log_user(user_or_id=None, username: str | None = None) -> str:
    if isinstance(user_or_id, User):
        user_id = user_or_id.id
        clean_name = clean_username(user_or_id.username, user_or_id.first_name or f"user_{user_id}")
    else:
        user_id = user_or_id
        clean_name = clean_username(username, f"user_{user_id}" if user_id else "N/A")
    if user_id:
        return f"@{clean_name} [{user_id}]"
    return f"@{clean_name}"


def format_log_chat(update: Update | None = None, chat_id: int | None = None) -> str:
    chat = update.effective_chat if update and update.effective_chat else None
    title = getattr(chat, "title", None) or "Grupo principal"
    resolved_chat_id = chat_id or getattr(chat, "id", None) or GROUP_CHAT_ID
    return f"{title} [{resolved_chat_id}]"


def get_update_message_link(update: Update | None) -> str | None:
    if not update:
        return None
    message = update.effective_message
    if update.callback_query and update.callback_query.message:
        message = update.callback_query.message
    return getattr(message, "link", None) if message else None


def build_log_message(event_type: str, fields: list[tuple[str, object]], emoji: str = "🧾", target_user_id: int | None = None) -> str:
    header = f"{emoji} <b>#{html.escape(event_type.upper())}</b>"
    lines = [header]
    for label, value in fields:
        if value is None or value == "":
            continue
        lines.append(f"• <b>{html.escape(str(label))}:</b> {html.escape(str(value))}")
    lines.append(f"• <b>Fecha:</b> {format_log_datetime()}")
    if target_user_id:
        lines.append(f"#id{target_user_id}")
    message = "\n".join(lines)
    if len(message) > MAX_LOG_MESSAGE_LENGTH:
        message = message[: MAX_LOG_MESSAGE_LENGTH - 3] + "..."
    return message


def insert_log_event_record(
    event_type: str,
    event_key: str | None,
        ...
        with get_db_connection() as conn:
            ...
            c.execute(
                ...
                INSERT INTO log_events
                    (event_type, event_key, actor_user_id, target_user_id, chat_id, message, created_at, delivered, delivered_message_id, delivery_error)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    clean_text(event_type, 64, "EVENTO"),
                    clean_text(event_key, 64, "") if event_key else None,
                    actor_user_id,
                    target_user_id,
                    chat_id,
                    clean_text(message, MAX_LOG_MESSAGE_LENGTH, ""),
                    datetime.now(timezone.utc).isoformat(),
                    delivered,
                    delivered_message_id,
                    clean_text(delivery_error, 500, "") if delivery_error else None,
                ),
            )
            return c.lastrowid
    except Exception as exc:
        logger.error("No se pudo registrar log_event en SQLite: %s", safe_error_text(exc))
        return None


def update_log_event_delivery(log_event_id: int | None, delivered_message_id: int | None = None, delivery_error: str | None = None) -> None:
    if not log_event_id:
        return
    try:
        with get_db_connection() as conn:
            ...
            c.execute(
                """
                UPDATE log_events
                SET delivered = ?, delivered_message_id = ?, delivery_error = ?
                WHERE id = ?
                """,
                (
                    1 if delivery_error is None else 0,
                    delivered_message_id,
                    clean_text(delivery_error, 500, "") if delivery_error else None,
                    log_event_id,
                ),
        ...
        logger.error("No se pudo actualizar entrega del log_event %s: %s", log_event_id, safe_error_text(exc))
...
async def send_log_event(
    ...
    *,
    ...
    if LOG_CHANNEL_ID == 0 or not is_log_category_enabled(category):
        return
    resolved_chat_id = chat_id or (update.effective_chat.id if update and update.effective_chat else GROUP_CHAT_ID)
    message = build_log_message(event_type, fields, emoji=emoji, target_user_id=target_user_id)
    log_event_id = insert_log_event_record(
        event_type,
        event_key,
        actor_user_id,
        target_user_id,
    ...
    reply_markup = None
    message_link = get_update_message_link(update)
    if message_link:
        reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("👀 Ir al mensaje", url=message_link)]])
    try:
        ...
        update_log_event_delivery(log_event_id, delivered_message_id=getattr(sent, "message_id", None))
    except TelegramError as exc:
        update_log_event_delivery(log_event_id, delivery_error=safe_error_text(exc, 500))
        logger.error("No se pudo enviar evento %s al LOG_CHANNEL_ID=%s: %s", event_type, LOG_CHANNEL_ID, safe_error_text(exc))
    except Exception as exc:
        update_log_event_delivery(log_event_id, delivery_error=safe_error_text(exc, 500))
        logger.error("Error inesperado enviando evento %s al canal de registro: %s", event_type, safe_error_text(exc))


def get_invite_link_fields(update: Update | None) -> list[tuple[str, object]]:
    chat_member_update = update.chat_member if update and update.chat_member else None
    invite_link = getattr(chat_member_update, "invite_link", None) if chat_member_update else None
    if not invite_link:
        return [("Enlace de", "No disponible")]
    creator = getattr(invite_link, "creator", None)
    link_name = getattr(invite_link, "name", None)
    fields = []
    if creator:
        fields.append(("Enlace de", format_log_user(creator)))
    else:
        fields.append(("Enlace de", "No disponible"))
    if link_name:
        fields.append(("Etiqueta", link_name))
    return fields
...
def status_label(status: str | None) -> str:
    labels = {
        ...
        ChatMember.ADMINISTRATOR: "administrador",
        ChatMember.MEMBER: "miembro",
        ChatMember.RESTRICTED: "restringido",
        ChatMember.LEFT: "fuera",
        ChatMember.BANNED: "baneado",
    }
    return labels.get(status, str(status or "desconocido"))


async def log_admin_status_change(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_member_update = update.chat_member
    if not chat_member_update or not LOG_ADMIN_CHANGE_EVENTS:
        return
    ...
    if status_change:
        changed_fields.append(f"estado {status_label(status_change[0])} → {status_label(status_change[1])}")
    for field in sorted(ADMIN_PRIVILEGE_FIELDS):
        if field in difference:
            old_value, new_value = difference[field]
            changed_fields.append(f"{field}: {old_value} → {new_value}")
    if not changed_fields:
        return
    admin_related = old_status in {ChatMember.ADMINISTRATOR, ChatMember.OWNER} or new_status in {ChatMember.ADMINISTRATOR, ChatMember.OWNER}
    if not admin_related and "custom_title" not in difference:
        return
    actor = getattr(chat_member_update, "from_user", None)
    target = new_member.user
    await send_log_event(
        context,
        "ADMIN_MODIFICADO",
        [
            ("De", format_log_user(actor) if actor else "Sistema/Telegram"),
            ("A", format_log_user(target)),
            ("Grupo", format_log_chat(update)),
            ("Cambio", "; ".join(changed_fields[:8])),
        ],
        emoji="🛡️",
...
async def plan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_log_event(
        ...
        "PLAN_ACTIVADO",
        [
            ("De", format_log_user(update.effective_user)),
            ("A", format_log_user(user_id, await get_username(user_id, context))),
            ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
            ("Días", days),
            ("Vence", end_date.astimezone(BOT_TIMEZONE).strftime("%Y-%m-%d %H:%M")),
        ],
        emoji="✅",
        category="membership",
        update=update,
...
async def extender_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_log_event(
        ...
        [
            ...
            ("A", format_log_user(user_id, await get_username(user_id, context))),
            ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
            ("Días añadidos", days_to_add),
            ("Nuevo vencimiento", new_end_date.astimezone(BOT_TIMEZONE).strftime("%Y-%m-%d %H:%M")),
        ],
        emoji="➕",
        category="membership",
        update=update,
...
async def menos_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_log_event(
        ...
        [
            ...
            ("A", format_log_user(user_id, await get_username(user_id, context))),
            ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
            ("Días restados", days_to_subtract),
            ("Nuevo vencimiento", new_end_date.astimezone(BOT_TIMEZONE).strftime("%Y-%m-%d %H:%M")),
        ],
        emoji="➖",
        category="membership",
...
async def expulsar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await send_log_event(
            ...
            [
                ("De", format_log_user(update.effective_user)),
                ("A", format_log_user(user_id_to_expel, await get_username(user_id_to_expel, context))),
                ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
                ("Acción", audit_action),
            ],
            emoji="👢",
            ...
            target_user_id=user_id_to_expel,
            event_key=audit_action,
        )
...
async def aceptar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await send_log_event(
            ...
            [
                ("De", format_log_user(update.effective_user)),
                ("A", format_log_user(user_id_to_accept, await get_username(user_id_to_accept, context))),
                ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
                ("Acción", audit_action),
            ],
            emoji="↩️",
            ...
            update=update,
            actor_user_id=actor_id,
            target_user_id=user_id_to_accept,
            event_key=audit_action,
        )
    except TelegramError as e:
...
async def mute_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_log_event(
        ...
        [
            ("De", format_log_user(update.effective_user)),
            ("A", format_log_user(target_user_id, target_username)),
            ("Grupo", format_log_chat(update, chat_id)),
            ("Duración", duration_label),
            ("Hasta", until_date.astimezone(BOT_TIMEZONE).strftime("%Y-%m-%d %H:%M") if until_date else "Indeterminado"),
        ],
        emoji="🔇",
        category="moderation",
        update=update,
    ...
    for user_id, username, duration_seconds, until_date_str in due_mutes:
        try:
            ...
            await send_log_event(
                ...
                [
                    ("De", "Sistema"),
                    ("A", format_log_user(user_id, username)),
                    ("Grupo", format_log_chat(chat_id=GROUP_CHAT_ID)),
                    ("Motivo", "Vencimiento del silencio"),
                ],
                emoji="🔊",
                category="moderation",
                actor_user_id=0,
...
async def info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_caller_admin:
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute("SELECT 1 FROM blacklist WHERE user_id = ? LIMIT 1", (target_user_id,))
            is_blacklisted_target = c.fetchone() is not None
        if is_blacklisted_target:
            reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Retirar de lista negra", callback_data=f"info_blacklist_remove_{target_user_id}")]])
        else:
            reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("🚫 Ban automático / Lista negra", callback_data=f"info_blacklist_add_{target_user_id}")]])
...
async def asignar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await restore_member_after_approval(context, target_user_id)
    ...
    await send_log_event(
        ...
        [
            ("De", format_log_user(update.effective_user)),
            ("A", format_log_user(target_user_id, username)),
            ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
            ("Rango", assigned_rank),
        ],
        emoji="🏷️",
...
async def activate_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_log_event(
        context,
        "PLAN_ACTIVADO_BIENVENIDA",
        [
            ("De", format_log_user(query.from_user)),
            ("A", format_log_user(target_user_id, username)),
            ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
            ("Días", days),
            ("Vence", end_date.astimezone(BOT_TIMEZONE).strftime("%Y-%m-%d %H:%M")),
...
async def get_chat_id_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = (f"<b>Información del Chat:</b>\n"
               f"  - Título: <code>{html.escape(str(title))}</code>\n"
               f"  - ID: <code>{chat_id}</code>\n"
               f"  - Tipo: <code>{html.escape(str(chat_type))}</code>")
    await update.message.reply_text(message, parse_mode=ParseMode.HTML)
...
def chat_type_label(chat_type: str | None) -> str:
    labels = {
        ...
        ChatType.PRIVATE: "Privado",
    }
    return labels.get(chat_type, str(chat_type or "desconocido"))


def bool_to_db(value) -> int | None:
    if value is None:
        return None
    return 1 if bool(value) else 0


def get_configured_chat_targets() -> list[tuple[int, str]]:
    targets: list[tuple[int, str]] = []
    seen: set[int] = set()
    for chat_id, source in (
        (GROUP_CHAT_ID, "GROUP_CHAT_ID"),
        (ADMIN_GROUP_CHAT_ID, "ADMIN_GROUP_CHAT_ID"),
        (REFERENCES_CHANNEL_ID, "REFERENCES_CHANNEL_ID"),
        (LOG_CHANNEL_ID, "LOG_CHANNEL_ID"),
    ):
        if chat_id and chat_id not in seen:
            targets.append((chat_id, source))
            seen.add(chat_id)
    return targets


def upsert_bot_chat_registry(record: dict) -> None:
    now_iso = datetime.now(timezone.utc).isoformat()
    values = {
        "chat_id": parse_int_field(record.get("chat_id"), min_value=-MAX_TELEGRAM_USER_ID, max_value=MAX_TELEGRAM_USER_ID),
        "title": clean_text(record.get("title"), 256, "Sin título"),
        "chat_type": clean_text(record.get("chat_type"), 32, "unknown"),
        "username": clean_username(record.get("username"), "") if record.get("username") else "",
        "bot_status": clean_text(record.get("bot_status"), 64, "unknown"),
        "is_admin": 1 if record.get("is_admin") else 0,
        "can_post_messages": bool_to_db(record.get("can_post_messages")),
        "can_invite_users": bool_to_db(record.get("can_invite_users")),
        "can_restrict_members": bool_to_db(record.get("can_restrict_members")),
        "can_delete_messages": bool_to_db(record.get("can_delete_messages")),
        "can_promote_members": bool_to_db(record.get("can_promote_members")),
        "first_seen_at": clean_text(record.get("first_seen_at"), 64, now_iso),
        "last_seen_at": clean_text(record.get("last_seen_at"), 64, now_iso),
        "last_admin_check_at": clean_text(record.get("last_admin_check_at"), 64, now_iso),
        "active": 1 if record.get("active", True) else 0,
        "source": clean_text(record.get("source"), 96, "observed"),
        "last_error": clean_text(record.get("last_error"), 500, ""),
    }
    if values["chat_id"] == 0:
        return
    with get_db_connection() as conn:
        ...
        c.execute(
            """
            INSERT INTO bot_chat_registry
                (chat_id, title, chat_type, username, bot_status, is_admin, can_post_messages, can_invite_users,
                 can_restrict_members, can_delete_messages, can_promote_members, first_seen_at, last_seen_at,
                 last_admin_check_at, active, source, last_error)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(chat_id) DO UPDATE SET
                ...
                first_seen_at = COALESCE(bot_chat_registry.first_seen_at, excluded.first_seen_at),
                last_seen_at = excluded.last_seen_at,
                last_admin_check_at = excluded.last_admin_check_at,
            ...
            (
                ...
                values["bot_status"],
                values["is_admin"],
                values["can_post_messages"],
                values["can_invite_users"],
                values["can_restrict_members"],
                values["can_delete_messages"],
                values["can_promote_members"],
                values["first_seen_at"],
                values["last_seen_at"],
                values["last_admin_check_at"],
                values["active"],
                values["source"],
                values["last_error"],
            ),
...
def mark_bot_chat_registry_inactive(chat_id: int, error_text: str) -> None:
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute(
            """
            UPDATE bot_chat_registry
            SET active = 0, is_admin = 0, last_admin_check_at = ?, last_error = ?
            WHERE chat_id = ?
            """,
            (datetime.now(timezone.utc).isoformat(), clean_text(error_text, 500, "No disponible"), chat_id),
        )
        conn.commit()


def bot_member_permissions(member) -> dict:
    return {
...
async def fetch_bot_chat_admin_record(context: ContextTypes.DEFAULT_TYPE, chat_id: int, source: str = "registry") -> dict | None:
    bot_user = await context.bot.get_me()
    try:
        chat = await context.bot.get_chat(chat_id=chat_id)
        bot_member = await context.bot.get_chat_member(chat_id=chat_id, user_id=bot_user.id)
    except TelegramError as exc:
        mark_bot_chat_registry_inactive(chat_id, safe_error_text(exc))
        logger.warning("No se pudo verificar chat %s para /idinfo: %s", chat_id, safe_error_text(exc))
        return None

    chat_type = getattr(chat, "type", None)
    status = getattr(bot_member, "status", "unknown")
    is_admin = status in BOT_ADMIN_STATUSES
    active = status not in {ChatMember.LEFT, ChatMember.BANNED}
    record = {
        "chat_id": chat.id,
        ...
        "last_admin_check_at": datetime.now(timezone.utc).isoformat(),
        "last_error": "",
    }
    record.update(bot_member_permissions(bot_member))
    upsert_bot_chat_registry(record)
...
async def refresh_configured_bot_chats(context: ContextTypes.DEFAULT_TYPE) -> None:
    for chat_id, source in get_configured_chat_targets():
        await fetch_bot_chat_admin_record(context, chat_id, source)


async def collect_bot_admin_chats(context: ContextTypes.DEFAULT_TYPE) -> list[dict]:
    await refresh_configured_bot_chats(context)
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute(
            """
            SELECT chat_id, source
            FROM bot_chat_registry
            WHERE active = 1
              AND chat_type IN ('group', 'supergroup', 'channel')
            ORDER BY is_admin DESC, last_seen_at DESC, chat_id
            LIMIT ?
            """,
            (IDINFO_MAX_KNOWN_CHATS,),
        )
        known_targets = c.fetchall()

    records: list[dict] = []
    seen: set[int] = set()
    for chat_id, source in known_targets:
        if chat_id in seen:
            continue
        seen.add(chat_id)
        record = await fetch_bot_chat_admin_record(context, chat_id, source or "registry")
        if record:
            records.append(record)

    records.sort(key=lambda item: (str(item.get("chat_type") != ChatType.CHANNEL), str(item.get("title", "")).lower(), item.get("chat_id", 0)))
    return records


def build_idinfo_page_from_records(records: list[dict], page: int) -> tuple[str, InlineKeyboardMarkup]:
    total = len(records)
    total_pages = max(1, -(-total // IDINFO_PAGE_SIZE))
    page = max(0, min(page, total_pages - 1))
    start_index = page * IDINFO_PAGE_SIZE
    end_index = start_index + IDINFO_PAGE_SIZE
    page_records = records[start_index:end_index]

    if not page_records:
        keyboard = [[InlineKeyboardButton("Cerrar 🗑️", callback_data="idinfo_close")]]
        return (
            "🆔 <b>Chats conocidos donde el bot es administrador</b>\n\n"
            "No hay grupos o canales conocidos donde el bot esté como administrador.\n\n"
            "ℹ️ El bot solo puede mostrar chats configurados o detectados desde que recibe actualizaciones <code>my_chat_member</code>.",
            InlineKeyboardMarkup(keyboard),
        )

    parts = [
        f"🆔 <b>Chats donde el bot es administrador</b>\nPágina {page + 1}/{total_pages} · Total: {total}\n\n"
    ]
    id_buttons = []
    for idx, record in enumerate(page_records, start_index + 1):
        chat_id = int(record["chat_id"])
        title = html.escape(record.get("title") or str(chat_id))
        chat_type = chat_type_label(record.get("chat_type"))
        username = record.get("username")
        username_line = f"\n   - <b>Username:</b> @{html.escape(username)}" if username else ""
        source_line = f"\n   - <b>Fuente:</b> {html.escape(record.get('source') or 'registry')}"
        parts.append(
            f"<b>{idx}.</b> {title}\n"
            f"   - <b>Tipo:</b> {html.escape(chat_type)}\n"
            f"   - <b>ID:</b> <code>{chat_id}</code>"
            f"{username_line}"
            f"{source_line}\n\n"
        )
        id_buttons.append([InlineKeyboardButton(f"Copiar/Ver ID · {idx}", callback_data=f"idinfo_chat_{chat_id}")])

    nav_buttons = []
    if page > 0:
        nav_buttons.append(InlineKeyboardButton("⬅️ Atrás", callback_data=f"idinfo_page_{page - 1}"))
    nav_buttons.append(InlineKeyboardButton("Cerrar 🗑️", callback_data="idinfo_close"))
    if end_index < total:
        nav_buttons.append(InlineKeyboardButton("Siguiente ➡️", callback_data=f"idinfo_page_{page + 1}"))

    keyboard = id_buttons + [nav_buttons]
    return "".join(parts), InlineKeyboardMarkup(keyboard)




# Panel avanzado de membresías y control operativo
MEMBERSHIP_CATEGORIES = {
    ...
    "no_plan": "Sin plan",
    "pending": "Pendientes",
    "manual_rank": "Rango manual",
    "absent_active": "Activos fuera del grupo",
}

RANK_PRICE_INFO = [
    ("Sombra", "$700 MXN al renovar", "Miembros nuevos o rango inicial."),
    ("Efebo", "$600 MXN al renovar", "Descuento de $100 MXN por antigüedad."),
    ("Hoplita", "$500 MXN al renovar", "Descuento acumulado por permanencia."),
    ("Héroe", "$400 MXN al renovar", "Reconocimiento por actividad y antigüedad."),
    ("Semi Dios", "$300 MXN al renovar", "Descuento alto por trayectoria."),
    ("Titan", "No sujeto a descuento", "Rango máximo; condiciones especiales a criterio del staff."),
]

URL_RE = re.compile(r"(?i)\b((?:https?://|www\.)\S+|(?:t\.me|telegram\.me|bit\.ly|tinyurl\.com|discord\.gg)/\S+)")

async def is_group_admin_user(context: ContextTypes.DEFAULT_TYPE, user_id: int) -> bool:
    if user_id in ADMIN_IDS:
        return True
    try:
...
async def restrict_member_until_approval(context: ContextTypes.DEFAULT_TYPE, user_id: int, reason: str = "pendiente de aprobación") -> None:
    ...
    try:
        ...
        logger.info("Usuario %s silenciado automáticamente: %s", user_id, reason)
    except TelegramError as exc:
        logger.error("No se pudo silenciar automáticamente a %s: %s", user_id, safe_error_text(exc))

async def restore_member_after_approval(context: ContextTypes.DEFAULT_TYPE, user_id: int) -> None:
    if not GROUP_CHAT_ID:
        return
    try:
        await context.bot.restrict_chat_member(
        ...
        logger.error("No se pudieron restaurar permisos de %s tras aprobación: %s", user_id, safe_error_text(exc))

async def ban_blacklisted_member_if_needed(new_member: User, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    if not get_bot_setting_bool("blacklist_auto_ban", True):
        return False
    if not is_user_blacklisted(new_member.id):
        return False
    if await ban_blacklisted_member_if_needed(new_member, update, context):
        return

    username = new_member.username or f"user_{new_member.id}"
    register_user(new_member.id, username)
    if get_bot_setting_bool("mute_new_members_until_approval", True):
        await restrict_member_until_approval(context, new_member.id, "nuevo miembro pendiente de aprobación")
    try:
        await context.bot.ban_chat_member(chat_id=GROUP_CHAT_ID, user_id=new_member.id)
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute("UPDATE users SET active = 0 WHERE tg_id = ?", (new_member.id,))
            c.execute("INSERT INTO expulsion_log (user_id, admin_id, action) VALUES (?, ?, 'auto_blacklist_ban')", (new_member.id, 0))
            conn.commit()
        await send_log_event(
            context,
            "BLACKLIST_AUTO_BAN",
            [("A", format_log_user(new_member)), ("Grupo", format_log_chat(update, GROUP_CHAT_ID)), ("Motivo", "Usuario registrado en lista negra")],
            emoji="🚫",
            category="moderation",
            update=update,
            actor_user_id=0,
            target_user_id=new_member.id,
            event_key="blacklist_auto_ban",
        )
        return True
    except TelegramError as exc:
        logger.error("No se pudo expulsar automáticamente a usuario en blacklist %s: %s", new_member.id, safe_error_text(exc))
        return True

async def handle_bot_join_attempt(new_member: User, actor: User | None, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    if not new_member.is_bot:
        return False
    actor_id = actor.id if actor else 0
    actor_is_admin = bool(actor_id and await is_group_admin_user(context, actor_id))
    if actor_is_admin:
        return True
    try:
        await context.bot.ban_chat_member(chat_id=GROUP_CHAT_ID, user_id=new_member.id)
        if actor_id:
            await restrict_member_until_approval(context, actor_id, "intento de ingresar bot sin autorización")
        await send_log_event(
            context,
            "BOT_NO_AUTORIZADO",
            [("Bot", format_log_user(new_member)), ("Agregado por", format_log_user(actor) if actor else "Desconocido"), ("Acción", "Bot expulsado; actor silenciado si fue posible")],
            emoji="🤖",
            category="security",
            update=update,
            actor_user_id=actor_id,
            target_user_id=new_member.id,
            event_key="unauthorized_bot_join",
        )
    except TelegramError as exc:
        logger.error("Error controlando bot no autorizado %s: %s", new_member.id, safe_error_text(exc))
    return True

async def link_guard_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not get_bot_setting_bool("link_guard_enabled", True):
        return
    message = update.effective_message
    user = update.effective_user
    if not message or not user or not update.effective_chat or update.effective_chat.id != GROUP_CHAT_ID:
        return
    if await is_group_admin_user(context, user.id) or has_limited_staff_permissions(user.id):
        return
    content = "\n".join(part for part in (message.text, message.caption) if part)
    if not URL_RE.search(content):
        return
    allowed_domains = get_allowed_link_domains()
    matched_links = [match.group(1) for match in URL_RE.finditer(content)]
    if allowed_domains:
        unauthorized = []
        for link in matched_links:
            cleaned = link.lower().replace("https://", "").replace("http://", "").replace("www.", "")
            domain = cleaned.split("/", 1)[0]
            if not any(domain == allowed or domain.endswith("." + allowed) for allowed in allowed_domains):
                unauthorized.append(link)
        if not unauthorized:
            return
    try:
        await message.delete()
    except TelegramError as exc:
        logger.error("No se pudo borrar link no autorizado: %s", safe_error_text(exc))
    until_date = datetime.now(timezone.utc) + timedelta(minutes=AUTO_LINK_MUTE_MINUTES)
    try:
        await context.bot.restrict_chat_member(
            chat_id=GROUP_CHAT_ID,
            user_id=user.id,
        ...
        log_moderation_audit(user.id, user.username or f"user_{user.id}", "auto_mute_link", AUTO_LINK_MUTE_MINUTES * 60, until_date, 0, "system")
    except TelegramError as exc:
        logger.error("No se pudo silenciar por link a %s: %s", user.id, safe_error_text(exc))
    await send_log_event(
        context,
        "LINK_NO_AUTORIZADO",
        [("De", format_log_user(user)), ("Grupo", format_log_chat(update, GROUP_CHAT_ID)), ("Acción", f"Mensaje borrado y mute {AUTO_LINK_MUTE_MINUTES} min")],
        emoji="🔗",
        category="security",
        update=update,
        actor_user_id=user.id,
        target_user_id=user.id,
        event_key="unauthorized_link",
    )
    raise ApplicationHandlerStop

def get_membership_rows(category: str) -> list[tuple]:
    now = datetime.now(timezone.utc)
    with get_db_connection() as conn:
        c = conn.cursor()
        if category == "active":
            c.execute("SELECT tg_id, username, active, end_date, manual_rank FROM users WHERE active = 1 ORDER BY username COLLATE NOCASE")
        elif category == "inactive":
            c.execute("SELECT tg_id, username, active, end_date, manual_rank FROM users WHERE active = 0 ORDER BY username COLLATE NOCASE")
        elif category == "expiring":
            c.execute("SELECT tg_id, username, active, end_date, manual_rank FROM users WHERE active = 1 AND end_date IS NOT NULL AND end_date BETWEEN ? AND ? ORDER BY end_date ASC", (now.isoformat(), (now + timedelta(days=3)).isoformat()))
        elif category == "expired":
            c.execute("SELECT tg_id, username, active, end_date, manual_rank FROM users WHERE active = 1 AND end_date IS NOT NULL AND end_date < ? ORDER BY end_date ASC", (now.isoformat(),))
        elif category == "no_plan":
            c.execute("SELECT tg_id, username, active, end_date, manual_rank FROM users WHERE end_date IS NULL OR active = 0 ORDER BY username COLLATE NOCASE")
        elif category == "pending":
            c.execute("SELECT p.user_id, p.username, COALESCE(u.active, 0), u.end_date, u.manual_rank FROM pending_new_members p LEFT JOIN users u ON u.tg_id = p.user_id WHERE p.approved = 0 ORDER BY p.join_time DESC")
        elif category == "manual_rank":
            c.execute("SELECT tg_id, username, active, end_date, manual_rank FROM users WHERE manual_rank IS NOT NULL AND manual_rank != '' ORDER BY username COLLATE NOCASE")
        else:
            c.execute("SELECT tg_id, username, active, end_date, manual_rank FROM users WHERE active = 1 ORDER BY username COLLATE NOCASE")
        return c.fetchall()

async def get_absent_active_members(context: ContextTypes.DEFAULT_TYPE) -> list[tuple]:
    rows = get_membership_rows("active")
    absent = []
    for user_id, username, active, end_date, manual_rank in rows[:MAX_MASS_ACTION_USERS]:
        try:
            member = await context.bot.get_chat_member(chat_id=GROUP_CHAT_ID, user_id=user_id)
            if member.status in {ChatMember.LEFT, ChatMember.BANNED}:
                absent.append((user_id, username, active, end_date, manual_rank))
        except TelegramError:
            absent.append((user_id, username, active, end_date, manual_rank))
        await asyncio.sleep(0.03)
    return absent

async def build_miembros_page(category: str, page: int, context: ContextTypes.DEFAULT_TYPE) -> tuple[str, InlineKeyboardMarkup]:
    rows = await get_absent_active_members(context) if category == "absent_active" else get_membership_rows(category)
    total = len(rows)
    total_pages = max(1, -(-total // MEMBERS_PAGE_SIZE))
    page = max(0, min(page, total_pages - 1))
    start = page * MEMBERS_PAGE_SIZE
    selected = rows[start:start + MEMBERS_PAGE_SIZE]
    label = MEMBERSHIP_CATEGORIES.get(category, category)
    parts = [f"👥 <b>Panel de miembros: {html.escape(label)}</b>\nPágina {page + 1}/{total_pages} · Total: {total}\n\n"]
    keyboard = []
    if not selected:
        parts.append("No hay miembros en esta categoría.")
    for user_id, username, active, end_date, manual_rank in selected:
        safe_username = html.escape(username or f"user_{user_id}")
        days_left = "N/A"
        end_dt = make_aware(end_date)
        if active and end_dt:
            days_left = f"{max(0, (end_dt - datetime.now(timezone.utc)).days)} días"
        parts.append(f"• @{safe_username} · <code>{user_id}</code> · {'Activo' if active else 'Inactivo'} · {days_left}\n")
        keyboard.append([InlineKeyboardButton(f"Gestionar @{safe_username[:24]}", callback_data=f"miembros_detail_{user_id}")])
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️", callback_data=f"miembros_cat_{category}_{page - 1}"))
    nav.append(InlineKeyboardButton("Cerrar", callback_data="miembros_close"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("➡️", callback_data=f"miembros_cat_{category}_{page + 1}"))
    keyboard.append(nav)
    return "".join(parts), InlineKeyboardMarkup(keyboard)

def build_miembros_category_panel() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Activos", callback_data="miembros_cat_active_0"), InlineKeyboardButton("Inactivos", callback_data="miembros_cat_inactive_0")],
        [InlineKeyboardButton("Por vencer", callback_data="miembros_cat_expiring_0"), InlineKeyboardButton("Vencidos", callback_data="miembros_cat_expired_0")],
        [InlineKeyboardButton("Sin plan", callback_data="miembros_cat_no_plan_0"), InlineKeyboardButton("Pendientes", callback_data="miembros_cat_pending_0")],
        [InlineKeyboardButton("Rango manual", callback_data="miembros_cat_manual_rank_0"), InlineKeyboardButton("Activos fuera", callback_data="miembros_cat_absent_active_0")],
        [InlineKeyboardButton("Cerrar", callback_data="miembros_close")],
    ])

async def miembros_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_admin_permissions(update, context):
        return
    await update.message.reply_text(
        "👥 <b>Gestión avanzada de membresías</b>\n\nElige una categoría. En cada miembro podrás sumar +1, +7, +15, +30 días o retirar el plan.",
        parse_mode=ParseMode.HTML,
        reply_markup=build_miembros_category_panel(),
    )

async def miembros_category_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
    ...
    await query.answer()
    try:
        raw_payload = query.data.replace("miembros_cat_", "", 1)
        category, page_str = raw_payload.rsplit("_", 1)
        page = int(page_str)
    except Exception:
        await query.edit_message_text("Solicitud inválida.")
        return
    text, markup = await build_miembros_page(category, page, context)
    await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=markup)

async def miembros_detail_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Solo administradores.", show_alert=True)
        return
    ...
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute("SELECT username, active, end_date, manual_rank FROM users WHERE tg_id = ?", (user_id,))
        row = c.fetchone()
    if not row:
        await query.edit_message_text("Usuario no encontrado en BD.")
        return
    username, active, end_date, manual_rank = row
    safe_username = html.escape(username or f"user_{user_id}")
    days_left = "N/A"
    end_dt = make_aware(end_date)
    if active and end_dt:
    ...
    text = (
        ...
        f"Rango manual: {html.escape(manual_rank or 'N/A')}"
    )
    keyboard = [
        [InlineKeyboardButton("+1", callback_data=f"miembros_add_{user_id}_1"), InlineKeyboardButton("+7", callback_data=f"miembros_add_{user_id}_7"), InlineKeyboardButton("+15", callback_data=f"miembros_add_{user_id}_15"), InlineKeyboardButton("+30", callback_data=f"miembros_add_{user_id}_30")],
        [InlineKeyboardButton("Retirar plan / inactivar", callback_data=f"miembros_remove_{user_id}")],
        [InlineKeyboardButton("Cerrar", callback_data="miembros_close")],
    ]
    await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(keyboard))

async def miembros_add_days_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Solo administradores.", show_alert=True)
        return
    try:
        _, _, user_id_str, days_str = query.data.split("_")
    ...
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute("SELECT username, end_date, active FROM users WHERE tg_id = ?", (user_id,))
        row = c.fetchone()
        if not row:
            await query.answer("Usuario no encontrado.", show_alert=True)
            return
        username, end_date, active = row
        current_end = make_aware(end_date)
        base = max(current_end or now, now)
        new_end = base + timedelta(days=days)
        c.execute("UPDATE users SET active = 1, end_date = ?, last_notification_date = NULL, activated_by_admin_id = ?, initial_days = COALESCE(initial_days, 0) + ? WHERE tg_id = ?", (new_end.isoformat(), query.from_user.id, days, user_id))
        conn.commit()
    await log_membership_audit(user_id, username or f"user_{user_id}", "panel_add", days, query.from_user.id, query.from_user.username or f"admin_{query.from_user.id}")
    await send_log_event(context, "PLAN_EXTENDIDO_PANEL", [("De", format_log_user(query.from_user)), ("A", format_log_user(user_id, username)), ("Días", days), ("Vence", new_end.astimezone(BOT_TIMEZONE).strftime("%Y-%m-%d %H:%M"))], emoji="➕", category="membership", update=update, actor_user_id=query.from_user.id, target_user_id=user_id, event_key="panel_add_days")
    await query.answer(f"Se agregaron {days} días.", show_alert=True)
    await miembros_detail_handler(update, context)

async def miembros_remove_plan_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Solo administradores.", show_alert=True)
        return
    ...
    with get_db_connection() as conn:
        ...
        c.execute("UPDATE users SET active = 0, end_date = NULL, last_notification_date = NULL WHERE tg_id = ?", (user_id,))
        conn.commit()
    await log_membership_audit(user_id, username, "panel_remove_plan", 0, query.from_user.id, query.from_user.username or f"admin_{query.from_user.id}")
    await send_log_event(context, "PLAN_RETIRADO", [("De", format_log_user(query.from_user)), ("A", format_log_user(user_id, username)), ("Acción", "Marcado como inactivo")], emoji="➖", category="membership", update=update, actor_user_id=query.from_user.id, target_user_id=user_id, event_key="panel_remove_plan")
    await query.answer("Plan retirado y usuario marcado inactivo.", show_alert=True)
    await miembros_detail_handler(update, context)

async def miembros_close_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Solo administradores.", show_alert=True)
        return
...
async def info_blacklist_add_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Solo administradores.", show_alert=True)
        return
    ...
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute("INSERT OR REPLACE INTO blacklist (user_id, username, reason, image_file_id, banned_by_admin_id, ban_date) VALUES (?, ?, ?, ?, ?, ?)", (user_id, username, "Agregado desde /info", "", query.from_user.id, datetime.now(timezone.utc).isoformat()))
        c.execute("UPDATE users SET active = 0 WHERE tg_id = ?", (user_id,))
        conn.commit()
    try:
        await context.bot.ban_chat_member(chat_id=GROUP_CHAT_ID, user_id=user_id)
    except TelegramError as exc:
        logger.warning("No se pudo banear desde /info a %s: %s", user_id, safe_error_text(exc))
    await send_log_event(context, "LISTA_NEGRA_INFO", [("De", format_log_user(query.from_user)), ("A", format_log_user(user_id, username)), ("Motivo", "Agregado desde /info")], emoji="🚫", category="moderation", update=update, actor_user_id=query.from_user.id, target_user_id=user_id, event_key="blacklist_info")
    await query.answer("Usuario agregado a lista negra y baneado si era posible.", show_alert=True)
    try:
        await query.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Retirar de lista negra", callback_data=f"info_blacklist_remove_{user_id}")]]))
    except Exception:
        pass

async def info_blacklist_remove_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Solo administradores.", show_alert=True)
        return
    ...
    with get_db_connection() as conn:
        ...
        conn.commit()
    try:
        await context.bot.unban_chat_member(chat_id=GROUP_CHAT_ID, user_id=user_id, only_if_banned=True)
    except TelegramError as exc:
        logger.warning("No se pudo desbanear desde /info a %s: %s", user_id, safe_error_text(exc))
    await send_log_event(context, "LISTA_NEGRA_RETIRADA", [("De", format_log_user(query.from_user)), ("A", format_log_user(user_id, username))], emoji="✅", category="moderation", update=update, actor_user_id=query.from_user.id, target_user_id=user_id, event_key="blacklist_remove_info")
    await query.answer("Usuario retirado de lista negra.", show_alert=True)
    try:
        await query.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚫 Ban automático / Lista negra", callback_data=f"info_blacklist_add_{user_id}")]]))
    except Exception:
        pass

async def rangos_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text_parts = ["🏛️ <b>Rangos informativos del grupo</b>\n\n"]
    for rank, price, legend in RANK_PRICE_INFO:
        text_parts.append(f"<b>{html.escape(rank)}</b> - {html.escape(price)}\n<i>{html.escape(legend)}</i>\n\n")
    text_parts.append("Los rangos son informativos y no otorgan permisos operativos.")
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Cerrar", callback_data="rangos_close")]])
    await update.message.reply_text("".join(text_parts), parse_mode=ParseMode.HTML, reply_markup=keyboard)

async def rangos_close_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    try:
        await query.message.delete()
...
async def daily_analytics_report_job(context: ContextTypes.DEFAULT_TYPE):
    if not get_bot_setting_bool("daily_report_enabled", True):
        return
    ...
    with get_db_connection() as conn:
        ...
        c.execute("SELECT COUNT(*) FROM pending_new_members WHERE approved = 0")
        pending_users = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM membership_audit WHERE timestamp >= ?", (since_iso,))
        membership_changes = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM moderation_audit WHERE timestamp >= ?", (since_iso,))
        moderation_changes = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM expulsion_log WHERE timestamp >= ?", (since_iso,))
        expulsions = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM blacklist WHERE COALESCE(ban_date, ban_timestamp) >= ?", (since_iso,))
        blacklist_additions = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM user_references WHERE reference_date >= ?", (since_iso,))
        references = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM runtime_errors WHERE timestamp >= ?", (since_iso,))
        errors = c.fetchone()[0]
    await send_log_event(
        context,
        "REPORTE_DIARIO",
        [("Activos", active_users), ("Inactivos", inactive_users), ("Pendientes", pending_users), ("Cambios de membresía", membership_changes), ("Moderación", moderation_changes), ("Expulsiones", expulsions), ("Altas lista negra", blacklist_additions), ("Referencias", references), ("Errores", errors)],
        emoji="📊",
        category="backup",
        actor_user_id=0,
...
# Plantillas y rollback de importación
TEMPLATE_DATASETS = {key: cfg for key, cfg in IMPORT_TABLE_CONFIGS.items()}

def build_plantillas_panel() -> InlineKeyboardMarkup:
    rows = []
    items = list(TEMPLATE_DATASETS.items())
    for idx in range(0, len(items), 2):
        row = []
        for key, cfg in items[idx:idx + 2]:
            row.append(InlineKeyboardButton(cfg["label"][:28], callback_data=f"plantilla_{key}"))
        rows.append(row)
    rows.append([InlineKeyboardButton("Cerrar", callback_data="plantillas_close")])
    return InlineKeyboardMarkup(rows)

async def plantillas_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_admin_permissions(update, context):
        return
    await update.message.reply_text("📄 <b>Plantillas CSV</b>\n\nElige una plantilla vacía con encabezados correctos para /importar.", parse_mode=ParseMode.HTML, reply_markup=build_plantillas_panel())
...
async def plantilla_export_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    key = query.data.replace("plantilla_", "", 1)
    cfg = TEMPLATE_DATASETS.get(key)
    if not cfg:
    ...
    try:
        ...
        with open(tmp_name, "rb") as file_obj:
            await context.bot.send_document(chat_id=query.message.chat.id, document=file_obj, filename=filename, caption=f"📄 Plantilla: {html.escape(cfg['label'])}", parse_mode=ParseMode.HTML)
    finally:
        if tmp_name:
            try:
                os.remove(tmp_name)
...
async def plantillas_close_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Solo administradores.", show_alert=True)
        return
...
async def deshacer_importacion_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_admin_permissions(update, context):
        return
    if not context.args:
        await update.message.reply_text("Uso: <code>/deshacerimportacion &lt;batch_id&gt;</code>", parse_mode=ParseMode.HTML)
        return
    batch_id = parse_int_field(context.args[0], 0, 1, 10**12)
    if not batch_id:
        await update.message.reply_text("❌ batch_id inválido.")
        return
    try:
        restored, deleted = rollback_import_batch(batch_id, update.effective_user.id)
        await update.message.reply_text(f"✅ Rollback aplicado. Restaurados: {restored}. Eliminados: {deleted}.")
    except Exception as exc:
        await update.message.reply_text(f"❌ No se pudo deshacer: {safe_error_text(exc)}", parse_mode=ParseMode.HTML)

# Configuración exclusiva del super administrador
CONFIG_TOGGLES = {
    "mute_new_members_until_approval": "Silenciar nuevos hasta aprobación",
    "blacklist_auto_ban": "Ban automático de lista negra",
    "link_guard_enabled": "Anti-links no autorizados",
    "daily_report_enabled": "Reporte diario al canal de registro",
    "strict_approval_enabled": "Expulsión automática sin aprobación",
}

def build_config_panel() -> tuple[str, InlineKeyboardMarkup]:
    parts = ["⚙️ <b>Configuración sensible de Samaritan</b>\n\nSolo el super administrador puede modificar este panel.\n\n"]
    keyboard = []
    for key, label in CONFIG_TOGGLES.items():
        enabled = get_bot_setting_bool(key, True)
        parts.append(f"• <b>{html.escape(label)}:</b> {'ON ✅' if enabled else 'OFF ❌'}\n")
        keyboard.append([InlineKeyboardButton(f"{'✅' if enabled else '❌'} {label}", callback_data=f"config_toggle_{key}")])
    parts.append("\nDominios autorizados para links: <code>" + html.escape(get_bot_setting("allowed_link_domains", "ninguno") or "ninguno") + "</code>")
    keyboard.append([InlineKeyboardButton("Cerrar", callback_data="config_close")])
    return "".join(parts), InlineKeyboardMarkup(keyboard)

async def config_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_super_admin_permissions(update, context):
        return
    text, markup = build_config_panel()
    await update.message.reply_text(text, parse_mode=ParseMode.HTML, reply_markup=markup)

async def config_toggle_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await check_super_admin_permissions(update, context):
        return
    key = query.data.replace("config_toggle_", "", 1)
    if key not in CONFIG_TOGGLES:
        await query.answer("Configuración inválida.", show_alert=True)
        return
    current = get_bot_setting_bool(key, True)
    set_bot_setting(key, "false" if current else "true", query.from_user.id)
    await query.answer("Configuración actualizada.")
    text, markup = build_config_panel()
    await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=markup)

async def config_close_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await check_super_admin_permissions(update, context):
        return
    await query.answer()
...
async def idinfo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_admin_permissions(update, context):
        return
    records = await collect_bot_admin_chats(context)
    context.user_data["idinfo_records"] = records
    text, reply_markup = build_idinfo_page_from_records(records, 0)
    await update.message.reply_text(text, parse_mode=ParseMode.HTML, reply_markup=reply_markup)


async def idinfo_pagination_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Esta acción solo puede ser realizada por un administrador.", show_alert=True)
        return
    ...
    try:
        ...
        await query.edit_message_text("Error al procesar la página de /idinfo.")
        return
    records = context.user_data.get("idinfo_records")
    if records is None:
        records = await collect_bot_admin_chats(context)
        context.user_data["idinfo_records"] = records
    text, reply_markup = build_idinfo_page_from_records(records, page)
    if query.message.text != text or query.message.reply_markup != reply_markup:
        await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=reply_markup)


async def idinfo_detail_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Esta acción solo puede ser realizada por un administrador.", show_alert=True)
        return
    try:
        ...
        await query.answer("ID inválido.", show_alert=True)
        return
    await query.answer(f"Chat ID: {chat_id}", show_alert=True)


async def idinfo_close_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await is_admin_check(update, context):
        await query.answer("Esta acción solo puede ser realizada por un administrador.", show_alert=True)
        return
    await query.answer()
    context.user_data.pop("idinfo_records", None)
    try:
        await query.message.delete()
    except Exception as e:
        logger.warning(f"No se pudo borrar el panel de /idinfo: {safe_error_text(e)}")


async def track_bot_chat_membership(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_member_update = update.my_chat_member
    if not chat_member_update:
        return
    chat = chat_member_update.chat
    new_member = chat_member_update.new_chat_member
    status = getattr(new_member, "status", "unknown")
    is_admin = status in BOT_ADMIN_STATUSES
    record = {
        "chat_id": chat.id,
    ...
    record.update(bot_member_permissions(new_member))
    upsert_bot_chat_registry(record)
...
async def send_welcome_message(new_member: Update.effective_user, update: Update, context: ContextTypes.DEFAULT_TYPE):
    if new_member.is_bot:
        actor = getattr(getattr(update, "chat_member", None), "from_user", None) or update.effective_user
        await handle_bot_join_attempt(new_member, actor, update, context)
        return
    ...
    context.job_queue.run_once(
        ...
        when=timedelta(seconds=NEW_MEMBER_APPROVAL_TIMEOUT_SECONDS),
    ...
    await send_log_event(
        ...
        "ENTRADA_USUARIO",
        [
            ("De", format_log_user(new_member)),
            ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
            *get_invite_link_fields(update),
        ],
        emoji="➕",
        category="member",
        update=update,
...
async def auto_expel_unapproved_member(context: ContextTypes.DEFAULT_TYPE):
    ...
    if result and result[0] == 0:  # No fue aprobado
        try:
            ...
            await send_log_event(
                context,
                "EXPULSION_AUTOMATICA",
                [
                    ("De", "Sistema"),
                    ("A", format_log_user(user_id, username)),
                    ("Grupo", format_log_chat(chat_id=chat_id)),
                    ("Motivo", "No fue aprobado en el tiempo límite"),
                ],
                emoji="🤖",
                category="moderation",
                actor_user_id=0,
                target_user_id=user_id,
                chat_id=chat_id,
                event_key="auto_expel_unapproved",
            )
        except TelegramError as e:
...
async def track_member_changes(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.chat_member or update.chat_member.chat.id != GROUP_CHAT_ID:
        return
    await log_admin_status_change(update, context)
    ...
    if not was_member and is_member:
        ...
        left_member = update.chat_member.new_chat_member.user
        actor = getattr(update.chat_member, "from_user", None)
        await send_log_event(
            context,
            "SALIDA_USUARIO",
            [
                ("De", format_log_user(left_member)),
                ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
                ("Acción generada por", format_log_user(actor) if actor else "Sistema/Telegram"),
            ],
            emoji="➖",
            category="member",
            update=update,
            actor_user_id=getattr(actor, "id", None),
            target_user_id=left_member.id,
            event_key="member_left",
        )
...
async def welcome_new_member_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for new_member in update.message.new_chat_members:
        if new_member.is_bot:
            await handle_bot_join_attempt(new_member, update.effective_user, update, context)
            continue
        await send_welcome_message(new_member, update, context)
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await send_log_event(
        ...
        [
            ("De", "Sistema"),
            ("Error", safe_error_text(context.error, 500)),
        ],
        emoji="🆘",
        category="error",
        event_key="runtime_error",
    )
...
ADMIN_MENU_COMMANDS = [
    ...
    {"command": "/idinfo", "description": "Ver chats/canales donde el bot es administrador y obtener su ID"},
]
    ...
    try:
        ...
        await send_log_event(
            ...
            [
                ("De", format_log_user(update.effective_user)),
                ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
                ("Referencias", references_count),
                ("Canal", REFERENCES_CHANNEL_ID),
            ],
            emoji="🖼️",
            category="member",
            update=update,
...
def build_archivo_panel() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("Canal de registro", callback_data="archivo_export_log_events"),
            InlineKeyboardButton("Chats del bot", callback_data="archivo_export_bot_chat_registry"),
        ],
...
def sanitize_import_value(table_key: str, column: str, value):
    ...
    id_columns = {"id", "tg_id", "user_id", "admin_id", "banned_by_admin_id", "activated_by_admin_id", "initial_days", "references_count", "rank_assigned_by_admin_id", "days", "posted_to_channel", "channel_message_id", "approved", "actor_user_id", "target_user_id", "chat_id", "delivered_message_id"}
    if column in id_columns:
        ...
        if column == "chat_id":
            return parse_int_field(value, 0, -MAX_TELEGRAM_USER_ID, MAX_TELEGRAM_USER_ID)
        if column == "days":
            return parse_int_field(value, 0, -9999, 9999)
        return parse_int_field(value, 0)
    if column in {"active", "is_admin", "can_post_messages", "can_invite_users", "can_restrict_members", "can_delete_messages", "can_promote_members"}:
        raw_flag = clean_text(value, 8, "")
        if raw_flag == "":
            return None if column.startswith("can_") else 0
        return 1 if parse_int_field(raw_flag, 0, 0, 1) else 0
    ...
    if column in {"event_type", "event_key"}:
        return clean_text(value, 64, "evento")
    if column in {"message", "delivery_error"}:
        return clean_text(value, MAX_LOG_MESSAGE_LENGTH, "")
    if column == "created_at":
        return clean_text(value, 64, datetime.now(timezone.utc).isoformat())
    if column == "delivered":
        return parse_int_field(value, 0, 0, 1)
    if column == "delivered_message_id":
        return parse_int_field(value, 0, 0, MAX_TELEGRAM_USER_ID)
...
def format_import_preview_row(table_key: str, row: dict[str, object]) -> str:
    ...
    if table_key == "log_events":
        return f"• {html.escape(str(row.get('event_type')))} · {html.escape(str(row.get('created_at')))}"
    if table_key == "bot_chat_registry":
        return f"• {html.escape(str(row.get('title') or 'Sin título'))} · <code>{html.escape(str(row.get('chat_id')))}</code> · {html.escape(str(row.get('chat_type') or 'N/A'))}"
...
def create_import_batch(table_key: str, file_name: str, admin_id: int) -> int:
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute(
            "INSERT INTO import_batches (table_key, file_name, admin_id, created_at) VALUES (?, ?, ?, ?)",
            (table_key, file_name, admin_id, datetime.now(timezone.utc).isoformat()),
        )
        return c.lastrowid

def update_import_batch(batch_id: int, imported_count: int, error_count: int) -> None:
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute("UPDATE import_batches SET imported_count = ?, error_count = ? WHERE batch_id = ?", (imported_count, error_count, batch_id))
        conn.commit()

def get_import_primary_key(config: dict) -> str:
    columns = tuple(config["columns"])
    for candidate in ("tg_id", "user_id", "chat_id", "event_name", "id"):
        if candidate in columns:
            return candidate
    return columns[0]

def backup_import_row(cursor: sqlite3.Cursor, batch_id: int, table_key: str, config: dict, sanitized: dict[str, object]) -> None:
    pk_column = get_import_primary_key(config)
    pk_value = sanitized.get(pk_column)
    if pk_value is None or pk_value == "":
        return
    table_name = quote_sql_identifier(config["table"])
    safe_pk = quote_sql_identifier(pk_column)
    cursor.execute(f"SELECT * FROM {table_name} WHERE {safe_pk} = ?", (pk_value,))
    existing = cursor.fetchone()
    cursor.execute(f"PRAGMA table_info({table_name})")
    table_columns = [row[1] for row in cursor.fetchall()]
    previous = dict(zip(table_columns, existing)) if existing else None
    cursor.execute(
        """
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (batch_id, table_key, pk_column, str(pk_value), json.dumps(previous, ensure_ascii=False) if previous else None, 1 if existing else 0, datetime.now(timezone.utc).isoformat()),
    )

async def import_rows_by_dataset(table_key: str, rows: list[dict[str, str]], batch_id: int | None = None) -> tuple[int, int]:
    ...
    with get_db_connection() as conn:
        ...
        for row in rows:
            try:
                sanitized = sanitize_import_row(table_key, row)
                if batch_id:
                    backup_import_row(c, batch_id, table_key, config, sanitized)
...
def rollback_import_batch(batch_id: int, admin_id: int) -> tuple[int, int]:
    ...
    with get_db_connection() as conn:
        ...
        c.execute("SELECT table_key, pk_column, pk_value, previous_row_json, was_existing FROM import_row_backups WHERE batch_id = ? ORDER BY id DESC", (batch_id,))
        backups = c.fetchall()
        for table_key, pk_column, pk_value, previous_row_json, was_existing in backups:
            config = IMPORT_TABLE_CONFIGS.get(table_key)
            if not config:
                continue
            table_name = quote_sql_identifier(config["table"])
            safe_pk = quote_sql_identifier(pk_column)
            if was_existing and previous_row_json:
                previous = json.loads(previous_row_json)
                columns = list(previous.keys())
                safe_columns = ", ".join(quote_sql_identifier(column) for column in columns)
                placeholders = ", ".join("?" for _ in columns)
                c.execute(f"INSERT OR REPLACE INTO {table_name} ({safe_columns}) VALUES ({placeholders})", [previous.get(column) for column in columns])
                restored += 1
            else:
                c.execute(f"DELETE FROM {table_name} WHERE {safe_pk} = ?", (pk_value,))
                deleted += 1
        c.execute("UPDATE import_batches SET rolled_back = 1, rolled_back_at = ?, rolled_back_by_admin_id = ? WHERE batch_id = ?", (datetime.now(timezone.utc).isoformat(), admin_id, batch_id))
        conn.commit()
...
async def importar_confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        batch_id = create_import_batch(table_key, file_name, query.from_user.id)
        imported_count, error_count = await import_rows_by_dataset(table_key, rows, batch_id=batch_id)
        update_import_batch(batch_id, imported_count, error_count)
        context.user_data.pop("pending_csv_import", None)
        message = (
            ...
            f"📁 <b>Archivo:</b> <code>{html.escape(file_name)}</code>\n"
            f"↩️ <b>Rollback:</b> <code>/deshacerimportacion {batch_id}</code>"
        )
        ...
        await send_log_event(
            ...
            [
                ("De", format_log_user(query.from_user)),
                ("Destino", get_importable_dataset_label(table_key)),
                ("Registros", imported_count),
                ("Errores", error_count),
                ("Archivo", file_name),
            ],
            emoji="📥",
...
async def seller_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action != "missing":
            await send_log_event(
                context,
                "ROL_INTERNO_MODIFICADO",
                [
                    ("De", format_log_user(update.effective_user)),
                    ("A", format_log_user(seller_id, username)),
                    ("Grupo", format_log_chat(update, GROUP_CHAT_ID)),
                    ("Rol", "seller" if action == "assigned" else "member"),
                    ("Acción", action),
                ],
                emoji="🏷️",
                category="admin_change",
...
def main():
    ...
    app.add_handler(MessageHandler((filters.TEXT | filters.CaptionRegex(URL_RE.pattern)) & filters.Chat(chat_id=GROUP_CHAT_ID), link_guard_message), group=-50)
    ...
    job_queue.run_daily(daily_analytics_report_job, time(DAILY_REPORT_HOUR, DAILY_REPORT_MINUTE, tzinfo=BOT_TIMEZONE), name="daily_analytics_report")
    ...
    app.add_handler(CommandHandler("idinfo", idinfo_command))
    app.add_handler(CommandHandler("miembros", miembros_command))
    app.add_handler(CallbackQueryHandler(miembros_category_handler, pattern=r"^miembros_cat_"))
    app.add_handler(CallbackQueryHandler(miembros_detail_handler, pattern=r"^miembros_detail_"))
    app.add_handler(CallbackQueryHandler(miembros_add_days_handler, pattern=r"^miembros_add_"))
    app.add_handler(CallbackQueryHandler(miembros_remove_plan_handler, pattern=r"^miembros_remove_"))
    app.add_handler(CallbackQueryHandler(miembros_close_handler, pattern=r"^miembros_close$"))
    app.add_handler(CallbackQueryHandler(info_blacklist_add_handler, pattern=r"^info_blacklist_add_"))
    app.add_handler(CallbackQueryHandler(info_blacklist_remove_handler, pattern=r"^info_blacklist_remove_"))
    app.add_handler(CommandHandler("rangos", rangos_command))
    app.add_handler(CallbackQueryHandler(rangos_close_handler, pattern=r"^rangos_close$"))
    app.add_handler(CommandHandler("plantillas", plantillas_command))
    app.add_handler(CallbackQueryHandler(plantilla_export_handler, pattern=r"^plantilla_"))
    app.add_handler(CallbackQueryHandler(plantillas_close_handler, pattern=r"^plantillas_close$"))
    app.add_handler(CommandHandler("deshacerimportacion", deshacer_importacion_command))
    app.add_handler(CommandHandler("config", config_command))
    app.add_handler(CallbackQueryHandler(config_toggle_handler, pattern=r"^config_toggle_"))
    app.add_handler(CallbackQueryHandler(config_close_handler, pattern=r"^config_close$"))
    app.add_handler(CallbackQueryHandler(idinfo_pagination_handler, pattern=r"^idinfo_page_"))
    app.add_handler(CallbackQueryHandler(idinfo_detail_handler, pattern=r"^idinfo_chat_"))
    app.add_handler(CallbackQueryHandler(idinfo_close_handler, pattern=r"^idinfo_close$"))
    ...
    app.add_handler(ChatMemberHandler(track_bot_chat_membership, ChatMemberHandler.MY_CHAT_MEMBER))
