...
def get_user_role(user_id: int | None) -> str:
    ...
    try:
        ...
        logger.error("Error al consultar rol interno para %s: %s", user_id, exc)
        return "member"
...
def is_user_blacklisted(user_id: int) -> bool:
    ...
    try:
        ...
        logger.error("Error al consultar blacklist para %s: %s", user_id, exc)
        return False
...
async def seller_can_act_on_target(
    ...
    try:
        ...
        logger.debug("No se pudo validar privilegios del objetivo %s antes de %s: %s", target_user_id, action_label, exc)
...
async def seller_can_activate_target(
    ...
    try:
        ...
        logger.debug("No se pudo validar target %s antes de activar bienvenida: %s", target_user_id, exc)
...
async def aceptar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Error al aceptar: {e}")
...
async def setadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        message += "\n<b>Comandos:</b>\n"
        message += "• <code>/seller &lt;ID/@username&gt;</code> - Asignar/remover seller\n"
        message += "• Sellers autorizados: <code>/aceptar</code>, <code>/expulsar</code> y botón <b>Activar 30 días</b>\n"
