...
async def seller_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_admin_permissions(update, context): return
    if len(context.args) != 1:
        await update.message.reply_text("Uso: <code>/seller &lt;ID_usuario&gt;</code>", parse_mode=ParseMode.HTML)
        return
    try:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("SELECT username, role FROM users LEFT JOIN user_roles ON users.tg_id = user_roles.user_id WHERE users.tg_id = ?", (seller_id,))
            result = c.fetchone()
            if not result:
                await update.message.reply_text(f"❌ Usuario {seller_id} no encontrado.")
                return
            username = result[0] or f"user_{seller_id}"
            current_role = result[1] if len(result) > 1 else "member"
            
            if current_role == "seller":
                c.execute("DELETE FROM user_roles WHERE user_id = ?", (seller_id,))
                await update.message.reply_text(f"✅ Rol de seller removido a @{username} (ID: {seller_id})")
            else:
                ...
                await update.message.reply_text(f"✅ Rol de seller asignado a @{username} (ID: {seller_id})")
            conn.commit()
    except Exception as e:
...
async def setplan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("SELECT * FROM membership_audit ORDER BY timestamp DESC LIMIT 5")
            audits = c.fetchall()
        ...
        for audit in audits:
            user_id, username, action, days, admin_id, admin_username, timestamp = audit[:7]
            message += f"• Usuario: @{html.escape(username or f'user_{user_id}')} (ID: {html.escape(str(user_id))})\n"
            message += f"  Acción: {html.escape(action)} ({days} días)\n"
            message += f"  Admin: @{html.escape(admin_username or f'admin_{admin_id}')} (ID: {html.escape(str(admin_id))})\n"
...
async def staff_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("SELECT username FROM user_roles WHERE role = 'seller'")
            sellers = c.fetchall()
        
        if sellers:
            for seller_username, in sellers:
                message += f"• <a href=\"https://t.me/{html.escape(seller_username)}\">@{html.escape(seller_username)}</a>\n"
        else:
            message += "• Ninguno\n"
