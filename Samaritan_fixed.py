...
async def ver_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for idx, (user_id, username, command, req_date, req_time) in enumerate(requests, 1):
            ...
            for row in csv_reader:
                try:
                    if table_type == "blacklist":
                        ...
                        request_date = row.get('request_date', datetime.now().strftime("%Y-%m-%d"))
                        request_time = row.get('request_time', datetime.now().strftime("%H:%M:%S"))
                        chat_type = row.get('chat_type', 'private')
                        c.execute("INSERT INTO bot_requests (user_id, username, command, request_date, request_time, chat_type) VALUES (?, ?, ?, ?, ?, ?)", (user_id, username, command, request_date, request_time, chat_type))
                        imported += 1
                except Exception as e:
...
async def staff_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        staff_message = """<blockquote><b>👑 STAFF DEL OLIMPO</b>

<b>• Administradores</b>
• @MrMxyzptlk04 - <i>El Men</i>
• @Fuzzcas2 - <i>Fuzz</i>
• @XnaxAK - <i>Alva</i>
• @Elkezo - <i>KEZO</i>
• @Hdzleg - <i>CO</i>
• @Carlosrdz19 - <i>STRIP</i>
