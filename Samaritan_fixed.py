...
logger.info(f"ADMIN_IDS cargados: {ADMIN_IDS}")
...
async def plan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Determinar dónde están los días. Si hay reply, los días son el primer argumento (índice 0).
    # Si no hay reply, y el primer argumento es un ID/username, los días son el segundo argumento (índice 1).
    # Si no hay reply y el primer argumento no es un ID/username (se asume que es el número de días), los días son el primer argumento (índice 0).
    days_arg_index = 0
    if not update.message.reply_to_message and context.args and (context.args[0].isdigit() or context.args[0].startswith('@')):
        # Si el primer argumento es un ID o username, entonces los días deben ser el segundo argumento.
        days_arg_index = 1
    
    if not user_id or len(context.args) <= days_arg_index or not context.args[days_arg_index].isdigit():
...
async def staff_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    staff_message = (
        ...
        "• <a href=\"https://t.me/Nomu181\">@Nomu181</a>"
    )
...
async def setadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        for admin_id in ADMIN_IDS:
            username = await get_username(admin_id, context) # Reutilizar la función get_username para consistencia
...
def main():
    ...
    logger.info(f"ADMIN_GROUP_CHAT_ID: {ADMIN_GROUP_CHAT_ID}")
    logger.info(f"ADMIN_IDS: {ADMIN_IDS}")
    logger.info(f"REFERENCES_CHANNEL_ID: {REFERENCES_CHANNEL_ID}")
    init_db()
