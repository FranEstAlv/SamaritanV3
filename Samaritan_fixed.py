...
class HealthCheckHandler(BaseHTTPRequestHandler):
    """Handler simple para healthchecks"""
    ...
    def do_GET(self):
        ...
        if self.path == '/health':
            ...
            self.end_headers()
            response = {"status": "ok", "service": "El Buen Samaritano"}
            self.wfile.write(json_module.dumps(response).encode())
        elif self.path == '/':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            response = {"message": "El Buen Samaritano Bot - Health Check Service"}
            self.wfile.write(json_module.dumps(response).encode())
        else:
...
def run_health_server():
    ...
    try:
        ...
        logger.info("Servidor HTTP de healthcheck iniciado en puerto 8000")
...
<b>Fecha:</b> {ban_date}"""
    ...
    def signal_handler(sig, frame):
        logger.info("Recibida señal de terminación. Deteniendo bot...")
        app.stop()
        sys.exit(0)
