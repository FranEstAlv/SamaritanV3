...
def parse_samaritan_csv(csv_content: str) -> dict:
    """Parsea un archivo CSV exportado por Samaritan con decoraciones y tags."""
    result = {'users': [], 'blacklist': [], 'bot_requests': [], 'errors': []}
    ...
    while i < len(lines):
        ...
        if line.startswith('TABLA:'):
            table_name = line.replace('TABLA:', '').strip().lower()
            if table_name == 'users':
                current_table = 'users'
            elif table_name == 'blacklist':
                current_table = 'blacklist'
            elif 'bot_requests' in table_name or 'bot requests' in table_name:
                current_table = 'bot_requests'
            i += 1
            continue
            ...
            if data_line:
                try:
                    ...
                    if len(values) >= len(current_headers):
                        ...
                        result[current_table].append(row)
                except Exception as e:
                    result['errors'].append(f"Línea {i}: {str(e)}")
...
async def log_request(update: Update):
    """Registra cada petición (comando) hecha al bot."""
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("""
                INSERT INTO bot_requests (user_id, username, command, date, time, chat_type)
                VALUES (?, ?, ?, ?, ?, ?)
...
ADMIN_MENU_COMMANDS = [
    ...
    {"command": "/listar", "description": "Ver lista negra con menú paginado"},
    ...
    {"command": "/importar", "description": "Importar datos desde CSV"},
]
...
async def list_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "Proporciona el ID o username del usuario (o responde a un mensaje):",
        parse_mode="HTML"
    )
    return BAN_USER_ID
...
async def ver_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Muestra las últimas n peticiones hechas al bot (1-50)."""
    if not await check_admin_permissions(update, context):
        return
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("""
                SELECT user_id, username, command, date, time
                FROM bot_requests
                ORDER BY id DESC
                LIMIT ?
            """, (n,))
        ...
        message_parts = [f"📋 <b>Últimas {len(requests)} Peticiones al Bot</b>\n\n"]
        for idx, (user_id, username, command, date, time) in enumerate(requests, 1):
            message_parts.append(
                f"<b>{idx}.</b> <code>{command}</code>\n"
...
# Comando para Importar CSV - Estados de conversación
IMPORT_WAITING_FILE = 1

async def importar_csv_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Inicia el proceso de importación de CSV."""
    if not await check_admin_permissions(update, context):
        return
    
    try:
        await update.message.reply_text(
            "📁 <b>IMPORTAR ARCHIVO CSV</b>\n\n"
            "Por favor, adjunta el archivo CSV que deseas importar.\n\n"
            "Formatos soportados:\n"
            "• <b>blacklist</b> - Lista de usuarios no permitidos\n"
            "• <b>users</b> - Información de usuarios\n"
            "• <b>bot_requests</b> - Registro de peticiones\n\n"
            "Usa /cancelar para cancelar la operación.",
            parse_mode=ParseMode.HTML
        )
...
async def importar_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Maneja la recepción del archivo CSV."""
    try:
        if not update.message.document:
            await update.message.reply_text(
                "❌ Debes adjuntar un archivo.\n\n"
                "Por favor, envía un archivo CSV válido."
            )
            return IMPORT_WAITING_FILE
        ...
        # Usar el parser inteligente que maneja decoraciones y tags
        parsed_data = parse_samaritan_csv(csv_content)
        
        # Determinar qué tablas tienen datos
        tables_with_data = []
        if parsed_data['users']:
            tables_with_data.append('users')
        if parsed_data['blacklist']:
            tables_with_data.append('blacklist')
        if parsed_data['bot_requests']:
            tables_with_data.append('bot_requests')
        
        if not tables_with_data:
            await update.message.reply_text(
                "❌ El archivo CSV no contiene datos válidos.\n\n"
                "Asegúrate de que el archivo sea exportado por el bot."
            )
            return ConversationHandler.END
        
        # Si hay múltiples tablas, usar la primera
        table_type = tables_with_data[0]
        analysis = {'users': parsed_data['users'], 'blacklist': parsed_data['blacklist'], 'bot_requests': parsed_data['bot_requests']}
        
        context.user_data['csv_content'] = csv_content
        context.user_data['table_type'] = table_type
        context.user_data['file_name'] = file_name
        context.user_data['analysis'] = analysis
        
        await show_import_preview(update, context, analysis, table_type, file_name)
        
        return ConversationHandler.END
        
    except Exception as e:
        await update.message.reply_text(f"❌ Error al procesar CSV: {e}")
        logger.error(f"Error en importar_file_handler: {e}")
        return ConversationHandler.END

async def importar_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Cancela la importación."""
    await update.message.reply_text("❌ Importación cancelada.")
    return ConversationHandler.END

async def analyze_csv_file(csv_content: str, table_type: str) -> dict:
    """Analiza un archivo CSV y retorna estadísticas."""
    # Opción 2: Limpiar saltos de línea problemáticos
    csv_content = csv_content.replace('\r\n', '\n')
    csv_content = csv_content.replace('\n\n', '\n')
    
    # Opción 1: Usar DictReader con quoting robusto
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            for row_num, row in enumerate(csv_reader, 1):
                try:
                    if table_type == "blacklist":
                        user_id = int(row.get('user_id', 0))
                        username = row.get('username', 'desconocido')
                        c.execute("SELECT * FROM blacklist WHERE user_id = ?", (user_id,))
                        existe = c.fetchone()
                        
                        if existe:
                            duplicados.append({'user_id': user_id, 'username': username, 'reason': row.get('reason', 'N/A')})
                        else:
                            nuevos.append({'user_id': user_id, 'username': username, 'reason': row.get('reason', 'N/A')})
                        total += 1
                    
                    elif table_type == "users":
                        tg_id = int(row.get('tg_id', 0))
                        username = row.get('username', 'desconocido')
                        c.execute("SELECT references_count FROM users WHERE tg_id = ?", (tg_id,))
                        existe = c.fetchone()
                        
                        if existe:
                            old_refs = existe[0]
                            new_refs = int(row.get('references_count', 0))
                            if old_refs != new_refs:
                                actualizables.append({'tg_id': tg_id, 'username': username, 'cambio': f'{old_refs} → {new_refs}'})
                            else:
                                duplicados.append({'tg_id': tg_id, 'username': username})
                        else:
                            nuevos.append({'tg_id': tg_id, 'username': username, 'referencias': row.get('references_count', 0)})
                        total += 1
                    
                    elif table_type == "bot_requests":
                        user_id = int(row.get('user_id', 0))
                        username = row.get('username', 'desconocido')
                        command = row.get('command', 'N/A')
                        nuevos.append({'user_id': user_id, 'username': username, 'command': command})
                        total += 1
    ...
    return {'total': total, 'nuevos': nuevos, 'duplicados': duplicados, 'actualizables': actualizables, 'errores': errores}

async def show_import_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, analysis: dict, table_type: str, file_name: str):
    """Muestra la vista previa del CSV con opciones de acción."""
    try:
        preview_text = (
            f"📊 <b>VISTA PREVIA DE IMPORTACIÓN</b>\n\n"
            f"📁 Archivo: <code>{file_name}</code>\n"
            f"📋 Tabla: <b>{table_type}</b>\n\n"
            f"📊 <b>Estadísticas:</b>\n"
            f"├─ Total de registros: <b>{analysis['total']}</b>\n"
            f"├─ 🆕 Registros nuevos: <b>{len(analysis['nuevos'])}</b>\n"
            f"├─ 📋 Registros duplicados: <b>{len(analysis['duplicados'])}</b>\n"
            f"├─ ⚠️ Registros actualizables: <b>{len(analysis['actualizables'])}</b>\n"
            f"└─ ❌ Errores: <b>{len(analysis['errores'])}</b>\n\n"
        )
        
        if analysis['nuevos']:
            preview_text += f"<b>🆕 Nuevos ({len(analysis['nuevos'])}):</b>\n"
            for item in analysis['nuevos'][:5]:
                if 'username' in item:
                    preview_text += f"  • @{item['username']} (ID: {item.get('user_id') or item.get('tg_id')})\n"
            if len(analysis['nuevos']) > 5:
                preview_text += f"  ... y {len(analysis['nuevos']) - 5} más\n"
            preview_text += "\n"
        
        if analysis['actualizables']:
            preview_text += f"<b>⚠️ Actualizables ({len(analysis['actualizables'])}):</b>\n"
            for item in analysis['actualizables'][:3]:
                preview_text += f"  • @{item['username']}: {item['cambio']}\n"
            if len(analysis['actualizables']) > 3:
                preview_text += f"  ... y {len(analysis['actualizables']) - 3} más\n"
            preview_text += "\n"
        
        keyboard = [
            [InlineKeyboardButton("✅ Importar Todo", callback_data="import_all"), InlineKeyboardButton("🆕 Solo Nuevos", callback_data="import_new_only")],
            [InlineKeyboardButton("📋 Ver Detalles", callback_data="import_details"), InlineKeyboardButton("❌ Cancelar", callback_data="import_cancel")]
        ]
        
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await update.message.reply_text(preview_text, parse_mode=ParseMode.HTML, reply_markup=reply_markup)
        
    except Exception as e:
        await update.message.reply_text(f"❌ Error mostrando preview: {e}")
        logger.error(f"Error en show_import_preview: {e}")

async def import_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Maneja las acciones de importación."""
    query = update.callback_query
    await query.answer()
    
    action = query.data
    
    if 'csv_content' not in context.user_data:
        await query.edit_message_text("❌ Sesión expirada. Por favor, intenta de nuevo con /importar")
        return
    
    csv_content = context.user_data['csv_content']
    ...
    try:
        if action == "import_all":
            await query.edit_message_text("⏳ Importando todos los registros...")
            imported, errors = await execute_import(csv_content, table_type, import_type="all")
            await show_import_result(query, imported, errors, file_name, table_type)
        
        elif action == "import_new_only":
            await query.edit_message_text("⏳ Importando solo registros nuevos...")
            imported, errors = await execute_import(csv_content, table_type, import_type="new_only")
            await show_import_result(query, imported, errors, file_name, table_type)
        
        elif action == "import_details":
            await show_detailed_preview(query, analysis, table_type)
        
        elif action == "import_cancel":
            await query.edit_message_text("❌ Importación cancelada.")
            context.user_data.pop('csv_content', None)
            context.user_data.pop('table_type', None)
            context.user_data.pop('file_name', None)
            context.user_data.pop('analysis', None)
        
        elif action == "import_back":
            await show_import_preview(update, context, analysis, table_type, file_name)
    
    except Exception as e:
        await query.edit_message_text(f"❌ Error: {e}")
        logger.error(f"Error en import_action_handler: {e}")

async def show_detailed_preview(query, analysis: dict, table_type: str):
    """Muestra detalles completos de la importación."""
    details_text = f"📋 <b>DETALLES DE IMPORTACIÓN - {table_type.upper()}</b>\n\n"
    
    if analysis['nuevos']:
        details_text += f"<b>🆕 REGISTROS NUEVOS ({len(analysis['nuevos'])}):</b>\n"
        for i, item in enumerate(analysis['nuevos'][:10], 1):
            if 'username' in item:
                details_text += f"{i}. @{item['username']} (ID: {item.get('user_id') or item.get('tg_id')})\n"
        if len(analysis['nuevos']) > 10:
            details_text += f"... y {len(analysis['nuevos']) - 10} más\n"
        details_text += "\n"
    
    if analysis['duplicados']:
        details_text += f"<b>📋 REGISTROS DUPLICADOS ({len(analysis['duplicados'])}):</b>\n"
        for i, item in enumerate(analysis['duplicados'][:5], 1):
            if 'username' in item:
                details_text += f"{i}. @{item['username']} (ID: {item.get('user_id') or item.get('tg_id')})\n"
        if len(analysis['duplicados']) > 5:
            details_text += f"... y {len(analysis['duplicados']) - 5} más\n"
        details_text += "\n"
    
    if analysis['actualizables']:
        details_text += f"<b>⚠️ REGISTROS ACTUALIZABLES ({len(analysis['actualizables'])}):</b>\n"
        for i, item in enumerate(analysis['actualizables'][:5], 1):
            details_text += f"{i}. @{item['username']}: {item['cambio']}\n"
        if len(analysis['actualizables']) > 5:
            details_text += f"... y {len(analysis['actualizables']) - 5} más\n"
    
    keyboard = [[InlineKeyboardButton("◀️ Volver", callback_data="import_back")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(details_text, parse_mode=ParseMode.HTML, reply_markup=reply_markup)

async def execute_import(csv_content: str, table_type: str, import_type: str = "all") -> tuple:
    """Ejecuta la importación según el tipo seleccionado."""
    imported = 0
    errors = 0
    ...
    try:
            ...
            for row in csv_reader:
                try:
                    if table_type == "blacklist":
                        user_id = int(row.get('user_id', 0))
                        username = row.get('username', 'desconocido')
                        reason = row.get('reason', 'Sin especificar')
                        image_file_id = row.get('image_file_id', '')
                        banned_by_admin_id = int(row.get('banned_by_admin_id', 0))
                        ban_date = row.get('ban_date', datetime.now().strftime("%Y-%m-%d"))
                        c.execute("SELECT * FROM blacklist WHERE user_id = ?", (user_id,))
                        existe = c.fetchone()
                        if import_type == "new_only" and existe:
                            continue
                        c.execute("INSERT OR REPLACE INTO blacklist (user_id, username, reason, image_file_id, banned_by_admin_id, ban_date) VALUES (?, ?, ?, ?, ?, ?)", (user_id, username, reason, image_file_id, banned_by_admin_id, ban_date))
                        imported += 1
                    elif table_type == "users":
                        tg_id = int(row.get('tg_id', 0))
                        username = row.get('username', 'desconocido')
                        ...
                        if import_type == "new_only" and existe:
                            continue
                        c.execute("INSERT OR REPLACE INTO users (tg_id, username, start_date, end_date, active, activated_by_admin_id, initial_days, references_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (tg_id, username, start_date, end_date, active, activated_by_admin_id, initial_days, references_count))
                        imported += 1
                    elif table_type == "bot_requests":
                        user_id = int(row.get('user_id', 0))
                        username = row.get('username', 'desconocido')
                        command = row.get('command', '')
                        date = row.get('date', row.get('request_date', datetime.now().strftime("%Y-%m-%d")))
                        time = row.get('time', row.get('request_time', datetime.now().strftime("%H:%M:%S")))
                        chat_type = row.get('chat_type', 'private')
                        c.execute("INSERT INTO bot_requests (user_id, username, command, date, time, chat_type) VALUES (?, ?, ?, ?, ?, ?)", (user_id, username, command, date, time, chat_type))
                        imported += 1
                except Exception as e:
        ...
        logger.error(f"Error en execute_import: {e}")
    return imported, errors

async def show_import_result(query, imported: int, errors: int, file_name: str, table_type: str):
    ...
    result_text = (
        ...
        f"📋 Tabla: <b>{table_type}</b>\n\n"
        f"📊 <b>Resultados:</b>\n"
        f"✅ Registros importados: <b>{imported}</b>\n"
...
# Comando para mostrar lista de staff
async def staff_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Muestra la lista de staff (administradores y sellers)."""
    try:
        staff_message = """<blockquote><b>👑 STAFF DEL OLIMPO
...
• @Nomu181</b></blockquote>"""
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                c = conn.cursor()
                c.execute("""
                    INSERT INTO bot_requests (user_id, username, command, date, time)
                    VALUES (?, ?, ?, ?, ?)
                """, (
            ...
            logger.error(f"Error registrando /staff: {e}")
...
# Comando para listar usuarios en lista negra con menú paginado
async def listar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Muestra la lista negra en un menú paginado."""
    if not await check_admin_permissions(update, context):
        return
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Error al obtener lista negra: {e}")
...
async def blacklist_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Volver a la lista desde detalles
            ...
            # Obtener página actual del contexto
...
async def archivo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Error al exportar base de datos: {e}")
        import traceback
        traceback.print_exc()
...
def main():
    ...
    # Comando para Listar usuarios en lista negra (oculto, solo admin)
    ...
    # Comando para Importar CSV (oculto, solo admin) - ConversationHandler
    importar_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("importar", importar_csv_command)],
        states={
            IMPORT_WAITING_FILE: [MessageHandler(filters.Document.ALL, importar_file_handler)]
        },
        fallbacks=[CommandHandler("cancelar", importar_cancel)],
        per_user=True,
        per_chat=True
    )
    app.add_handler(importar_conv_handler, group=-1)
    
    # Callback handlers para importar
    app.add_handler(CallbackQueryHandler(import_action_handler, pattern=r"^import_"))
