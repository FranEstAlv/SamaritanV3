...
def get_bot_requests_sync(n: int):
    """Obtiene peticiones de forma sincrónica para usar en executor."""
    try:
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("""
        ...
        logger.error(f"Error en get_bot_requests_sync: {e}")
        return []
...
async def ver_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Usar executor para no bloquear el loop asincrónico
        loop = asyncio.get_event_loop()
        requests = await loop.run_in_executor(None, lambda: get_bot_requests_sync(n))
        ...
        # Crear teclado con paginación si hay muchas peticiones
        keyboard = []
        if len(requests) > 5:
            keyboard.append([
                InlineKeyboardButton("◀️ Anterior", callback_data="ver_prev"),
                InlineKeyboardButton("Siguiente ▶️", callback_data="ver_next")
            ])
        keyboard.append([InlineKeyboardButton("❌ Cerrar", callback_data="ver_close")])
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await update.message.reply_text("".join(message_parts), parse_mode=ParseMode.HTML, reply_markup=reply_markup)
...
# Comando para Importar CSV - Paso 1
async def importar_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Inicia el proceso de importación en 2 pasos."""
    if not await check_admin_permissions(update, context):
        return
    ...
    await update.message.reply_text(
        ...
        "• <code>users</code> - Información de usuarios\n"
        "• <code>blacklist</code> - Lista negra\n"
        "• <code>bot_requests</code> - Registro de peticiones\n\n"
        "Usa /cancelar para cancelar.",
        parse_mode=ParseMode.HTML
    )
    return 1  # Siguiente estado

# Comando para Importar CSV - Paso 2
async def importar_file_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Procesa el archivo CSV adjunto."""
    try:
        # Usar effective_message para detectar documentos en cualquier contexto
        ...
        # Eliminar espacios en nombres de columnas para validación correcta
        fieldnames = set(col.strip() for col in csv_reader.fieldnames)
        csv_reader = csv.DictReader(io.StringIO(csv_content))
...
def main():
    ...
    # Comando para Importar CSV - ConversationHandler de 2 pasos
    importar_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("importar", importar_start_command)],
        states={
            1: [MessageHandler(filters.Document.ALL, importar_file_command)]
        },
        fallbacks=[CommandHandler("cancelar", cancel_conversation)],
        per_user=True,
        per_chat=True
    )
    app.add_handler(importar_conv_handler)
...
<b>Fecha:</b> {ban_date}"""
        ...
        # Usar edit_message_text para no crear mensajes duplicados
        try:
            if image_file_id:
                # Si hay imagen, no podemos editar (limitación de Telegram)
                # Entonces eliminamos y creamos nuevo
                ...
                # Si no hay imagen, podemos editar el mensaje existente
                await update.callback_query.edit_message_text(
            ...
            logger.error(f"Error al mostrar detalle: {e}")
            # Si falla, intentar crear nuevo mensaje
            try:
