...
async def info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        c = conn.cursor()
        c.execute("SELECT tg_id, username, start_date, end_date, active, activated_by_admin_id, initial_days FROM users WHERE tg_id = ?", (target_user_id,))
    ...
    tg_id, username, start_str, end_str, active, admin_id, initial_days = target_user_data
...
async def mensaje_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    is_admin_group = update.effective_chat.id == GROUP_CHAT_ID
...
async def ban_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.user_data["ban_state"] = "waiting_user_id"
    context.user_data["ban_data"] = {}
...
async def handle_ban_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Maneja todos los inputs del flujo de /ban."""
    if "ban_state" not in context.user_data:
        return
    
    state = context.user_data.get("ban_state")
    ban_data = context.user_data.get("ban_data", {})
    
    if state == "waiting_user_id":
        # Procesar ID o username
        user_input = update.message.text.strip()
        
        if user_input.startswith("@"):
            ban_data["username"] = user_input[1:]
            ban_data["user_id"] = None
        elif user_input.isdigit():
            ban_data["user_id"] = int(user_input)
            ban_data["username"] = None
        elif update.message.reply_to_message:
            ban_data["user_id"] = update.message.reply_to_message.from_user.id
            ban_data["username"] = update.message.reply_to_message.from_user.username
        else:
            await update.message.reply_text("❌ Formato inválido. Usa: ID, @username o responde a un mensaje.")
            return
        
        context.user_data["ban_data"] = ban_data
        context.user_data["ban_state"] = "waiting_reason"
        await update.message.reply_text("📝 Ahora envía el motivo del baneo:")
    
    elif state == "waiting_reason":
        # Procesar motivo
        ban_data["reason"] = update.message.text
        context.user_data["ban_data"] = ban_data
        context.user_data["ban_state"] = "waiting_image"
        await update.message.reply_text("🖼️ Ahora envía la imagen de prueba/evidencia del usuario:")

async def handle_ban_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Maneja el envío de imagen en el flujo de /ban."""
    if context.user_data.get("ban_state") != "waiting_image":
        return
    ...
    username = ban_data.get("username", "desconocido")
    ...
    # Limpiar estado
    context.user_data.pop("ban_state", None)
    context.user_data.pop("ban_data", None)
...
def main():
    ...
    mensaje_conv_handler = ConversationHandler(
        ...
        states={ENVIAR_MENSAJE: [MessageHandler((filters.TEXT | filters.PHOTO) & ~filters.COMMAND, handle_mensaje_input)]},
    ...
    app.add_handler(todos_conv_handler)
    app.add_handler(limpieza_conv_handler)
    # Lista Negra (/ban y /consulta) - Handlers simples sin ConversationHandler
    app.add_handler(CommandHandler("ban", ban_command))
    app.add_handler(CommandHandler("consulta", consulta_command))
    # Handlers para el flujo de /ban (deben ir ANTES de otros MessageHandlers)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVATE, handle_ban_input))
    app.add_handler(MessageHandler(filters.PHOTO & filters.ChatType.PRIVATE, handle_ban_photo))
    ...
    app.add_handler(mensaje_conv_handler)
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Chat(chat_id=GROUP_CHAT_ID), notify_user_on_message))
