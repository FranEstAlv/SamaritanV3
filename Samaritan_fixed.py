...
# Mssj
ENVIAR_MENSAJE, CONFIRM_PURGE, CONFIRM_TODOS = range(3)
...
async def activate_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        conn.commit()
    await query.answer(f"¡Plan de {days} días activado para el usuario {target_user_id}!", show_alert=True)
    try:
        await query.message.delete()
    except Exception as e:
        logger.error(f"No se pudo borrar el mensaje de bienvenida tras la activación: {e}")
...
async def mensaje_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if GROUP_CHAT_ID == 0:
        ...
        return ConversationHandler.END
    await update.message.reply_text("Escribe el mensaje a enviar al grupo. Envía /cancelar para abortar.")
...
def main():
    ...
    mensaje_conv_handler = ConversationHandler(
        ...
        states={ENVIAR_MENSAJE: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_mensaje_input)]},
