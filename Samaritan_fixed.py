...
VALID_MEMBER_RANKS = ("Sombra", "Efebo", "Hoplita", "Semi Dios", "Titan")
RANK_ALIASES = {
    "sombra": "Sombra",
    "efebo": "Efebo",
    "hoplita": "Hoplita",
...
async def security_guard_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_valid_callback_data(query.data):
        ...
        raise ApplicationHandlerStop
    if query.data.startswith(("estado_", "admin_menu_", "ver_", "setplan_", "listar_")) and user_id not in ADMIN_IDS:
        ...
        with get_db_connection() as conn:
            ...
            for row in rows:
                try:
                    ...
                    manual_rank = normalize_member_rank(row.get('manual_rank'))
                    rank_assigned_by_admin_id = parse_int_field(row.get('rank_assigned_by_admin_id'), 0)
                    rank_assigned_at = clean_text(row.get('rank_assigned_at'), 64, "")
                    c.execute("""
                        INSERT OR REPLACE INTO users 
                        (tg_id, username, start_date, end_date, active, activated_by_admin_id, initial_days, references_count, first_seen_at, manual_rank, rank_assigned_by_admin_id, rank_assigned_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (tg_id, username, start_date, end_date, active, activated_by_admin_id, initial_days, references_count, first_seen_at, manual_rank, rank_assigned_by_admin_id, rank_assigned_at))
                    imported += 1
                except Exception as e:
