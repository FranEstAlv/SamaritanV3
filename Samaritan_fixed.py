...
async def mensaje_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if update.effective_chat.type != ChatType.PRIVATE:
        await update.message.reply_text("❌ Este comando solo funciona en el chat privado del bot.")
        return ConversationHandler.END
...
async def ban_image_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("INSERT OR REPLACE INTO blacklist (user_id, username, reason, image_file_id, banned_by_admin_id, ban_date) VALUES (?, ?, ?, ?, ?, ?)",
                      (user_id, username, reason, photo_file_id, admin_id, ban_date))
            conn.commit()
        await update.message.reply_text(f"✅ Usuario {username or user_id} añadido a la lista negra.")
...
def main():
    ...
    # Lista Negra (/ban y /consulta)
    ban_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("ban", ban_command)],
        states={
            BAN_USER_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, ban_user_id_handler)],
            BAN_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, ban_reason_handler)],
            BAN_IMAGE: [MessageHandler(filters.PHOTO, ban_image_handler)]
        },
        fallbacks=[CommandHandler("cancelar", cancel_conversation)],
